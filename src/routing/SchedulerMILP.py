...
class SchedulerMILP:
    ...
    def setupMILP(self, K: int, P: int):
        ...
        for u in self.simulation.drones.keys():
            sum = [(1.0, delta[f"{u},{k}"]) for k in range(K)]
        ...
        for u in self.simulation.drones.keys():
            for s in self.simulation.stations.keys():
                for k1 in range(K):
                    ...
                    self.model.addConstr(gp.LinExpr(sum) >= lb, f"droneflow2a_{u},{s},{k}")
                    self.model.addConstr(gp.LinExpr(sum) <= ub, f"droneflow2b_{u},{s},{k}")
        ...
        for u in self.simulation.drones.keys():
            for k in range(K):
                sum = [(1.0, y[f"{u},{k},{d},{p}"]) for d in self.simulation.deliveries.keys() for p in range(P)]
        ...
        for d in self.simulation.deliveries.keys():
            for p in range(P):
                sum = [(1.0, y[f"{u},{k},{d},{p}"]) for u in self.simulation.drones.keys() for k in range(K)]
        ...
        for d in self.simulation.deliveries.keys():
            for p in range(P):
                sum = [(1.0, f[f"{d},{p},{i},{j}"]) for (i,j) in self.simulation.edges] + \
                      [(-1.0, y[f"{u},{k},{d},{p}"]) for u in self.simulation.drones.keys() for k in range(K)]
                ...
                for (i, j) in self.simulation.edges:
                    ...
                    sum += [(1.0, B[f"{u},{k}"]), (-CONSUMPTION_UPPER_BOUND, gamma[f"{u},{k}"]), (CONSUMPTION_UPPER_BOUND, x[f"{u},{k},{i},{j}"])] + \
                           [(self.simulation.cost(i, j, self.simulation.deliveries[d].weight), y[f"{u},{k},{d},{p}"])
                            for d in self.simulation.deliveries.keys() for p in range(P)]
        ...
        for d in self.simulation.deliveries.keys():
            for p in range(1, P):
                for u in self.simulation.drones.keys():
                    for k in range(K):
                        sum = [(self.simulation.time(i, j), y[f"{u},{k},{d},{p}"]) for (i, j) in
                               self.simulation.edges]
    ...
    def saveMILP(self):
        ...
        if not os.path.exists(self.simulation.outFOLDER): os.mkdir(self.simulation.outFOLDER)
        print(f"\t Saving MILP in {self.simulation.outFOLDER}/model.lp")
        self.model.write(f"{self.simulation.outFOLDER}/model.lp")
    ...
    def extractSolution(self):
        for u in self.simulation.drones.keys():
            for k in range(self.K):
                ...
                self.simulation.drones[u].schedule[k] = action
